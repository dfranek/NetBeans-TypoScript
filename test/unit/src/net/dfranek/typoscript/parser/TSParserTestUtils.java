 ...
 */
public class TSParserTestUtils extends TestCase {

	public static void assertNodeEquals(TSASTNode node, String nodeName, TSASTNodeType type, String value, int size) {
		assertEquals(node.getName(), nodeName);
		assertEquals(node.getType(), type);
		assertEquals(node.getValue(), value);
		assertTrue(node.getChildren().size() == size);
	}
}
